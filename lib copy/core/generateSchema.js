...
exports.generateSchemaVariableStatement = void 0;
 ...
 * ```
 */
function generateSchemaVariableStatement({ node, typeName, sourceFile, varName, schemaImportValue = 'S', getDependencyName = (identifierName) => (0, case_1.camel)(`${identifierName}Schema`), skipParseJSDoc = false, }) {
    let schema;
    let dependencies = [];
    let requiresImport = false;
    if (!node) {
        if (const_1.standardBuiltInObjects.includes(typeName)) {
    ...
    return {
        ...
        statement: f.createVariableStatement(node === null || node === void 0 ? void 0 : node.modifiers, f.createVariableDeclarationList([
            f.createVariableDeclaration(f.createIdentifier(varName), undefined, undefined, schema),
        ], ts.NodeFlags.Const)),
        requiresImport,
    };
}
exports.generateSchemaVariableStatement = generateSchemaVariableStatement;
function buildEffectSchemaProperties({ members, schemaImportValue: S, sourceFile, dependencies, getDependencyName, skipParseJSDoc, }) {
    ...
    members.forEach((member) => {
        ...
        properties.set(member.name, buildEffectSchemaPrimitive({
            S,
...
function buildEffectSchemaPrimitive({ S, typeNode, isOptional, isNullable, isPartial, isRequired, jsDocTags, sourceFile, dependencies, getDependencyName, skipParseJSDoc, }) {
    const effectSchemaProperties = (0, jsDocTags_1.jsDocTagToEffectSchemaProperties)(jsDocTags, isOptional, Boolean(isPartial), Boolean(isRequired), Boolean(isNullable));
    ...
    if (ts.isTypeReferenceNode(typeNode) && ts.isIdentifier(typeNode.typeName)) {
        ...
        if (identifierName === 'ReadonlyArray' && typeNode.typeArguments) {
            return buildEffectSchema(S, 'array', [
        ...
        if (identifierName === 'Record' && typeNode.typeArguments) {
            if (typeNode.typeArguments.length !== 2 ||
                ...
                throw new Error(`Record<${typeNode.typeArguments[0].getText(sourceFile)}, …> are not supported (https://github.com/effect-ts/schema#records)`);
            }
            return buildEffectSchema(S, 'record', [
                f.createPropertyAccessExpression(f.createIdentifier(S), f.createIdentifier('string')),
                buildEffectSchemaPrimitive({
                    S,
        ...
        if (identifierName === 'Set' && typeNode.typeArguments) {
            return buildEffectSchema(S, 'set', typeNode.typeArguments.map((i) => buildEffectSchemaPrimitive({
                S,
                typeNode: i,
                isOptional: false,
                jsDocTags,
        ...
        if (['Omit', 'Pick'].includes(identifierName) && typeNode.typeArguments) {
            ...
            return f.createCallExpression(f.createPropertyAccessExpression(buildEffectSchemaPrimitive({
                S,
        ...
        const effectSchema = f.createIdentifier(dependencyName);
    ...
    if (ts.isUnionTypeNode(typeNode)) {
        ...
        const values = nodes.map((i) => buildEffectSchemaPrimitive({
            S,
            typeNode: i,
            isOptional: false,
            isNullable: false,
    ...
    if (ts.isTupleTypeNode(typeNode)) {
        const values = typeNode.elements.map((i) => buildEffectSchemaPrimitive({
            S,
    ...
    if (ts.isLiteralTypeNode(typeNode)) {
        if (ts.isStringLiteral(typeNode.literal)) {
            return buildEffectSchema(S, 'literal', [f.createStringLiteral(typeNode.literal.text)], effectSchemaProperties);
        }
        if (ts.isNumericLiteral(typeNode.literal)) {
            return buildEffectSchema(S, 'literal', [f.createNumericLiteral(typeNode.literal.text)], effectSchemaProperties);
        }
        if (typeNode.literal.kind === ts.SyntaxKind.TrueKeyword) {
            return buildEffectSchema(S, 'literal', [f.createTrue()], effectSchemaProperties);
        }
        if (typeNode.literal.kind === ts.SyntaxKind.FalseKeyword) {
            return buildEffectSchema(S, 'literal', [f.createFalse()], effectSchemaProperties);
        }
    }
    ...
    if (ts.isTypeReferenceNode(typeNode) &&
        ...
        return buildEffectSchema(S, 'literal', [
    ...
    if (ts.isTypeLiteralNode(typeNode)) {
        return withEffectSchemaProperties(S, buildEffectSchemaObject({
            typeNode,
            S,
            sourceFile,
            dependencies,
    ...
    if (ts.isIntersectionTypeNode(typeNode)) {
        ...
        return rest.reduce((intersectionSchema, node) => (0, commonSchema_1.callPipe)(undefined, [
            intersectionSchema,
            (0, commonSchema_1.callCreateCallExpression)(S, 'extend', undefined, [
                buildEffectSchemaPrimitive({
                    S,
                    typeNode: node,
                    isOptional: false,
    ...
    if (ts.isLiteralTypeNode(typeNode)) {
        return buildEffectSchema(S, typeNode.literal.getText(sourceFile), [], effectSchemaProperties);
    }
 ...
 */
function buildEffectSchema(S, callName, args, properties) {
    const expression = f.createPropertyAccessExpression(f.createIdentifier(S), f.createIdentifier(callName));
    const effect = const_1.primitivePropertyList.includes(expression.name.escapedText)
        ? expression
        : f.createCallExpression(expression, undefined, args);
    return withEffectSchemaProperties(S, effect, properties);
}
function buildEffectSchemaExtendedSchema(S, schemaList, args, properties) {
    let effectSchemaCall = f.createIdentifier(schemaList[0]);
    for (let i = 1; i < schemaList.length; i++) {
        effectSchemaCall = f.createCallExpression(f.createPropertyAccessExpression(effectSchemaCall, f.createIdentifier('extend')), undefined, [
            (0, commonSchema_1.callCreateCallExpression)(S, 'getPropertySignatures', undefined, [
                f.createIdentifier(schemaList[i]),
            ]),
        ]);
    }
    if (args === null || args === void 0 ? void 0 : args.length) {
        effectSchemaCall = f.createCallExpression(f.createPropertyAccessExpression(effectSchemaCall, f.createIdentifier('extend')), undefined, args);
    }
 ...
 * @param properties
 */
function withEffectSchemaProperties(s, expression, properties = []) {
    return properties.reduce((expressionWithProperties, property) => {
        var _a, _b;
        if (property.identifier === 'optional.withDefault') {
            const isOptional = ((_b = (_a = expressionWithProperties === null || expressionWithProperties === void 0 ? void 0 : expressionWithProperties.expression) === null || _a === void 0 ? void 0 : _a.name) === null || _b === void 0 ? void 0 : _b.escapedText) === 'optional';
            const optionalExpression = isOptional
                ? expressionWithProperties
                : (0, commonSchema_1.callCreateCallExpression)(s, 'optional', undefined, [
                    expressionWithProperties,
                ]);
            return (0, commonSchema_1.callCreatePropertyAccessExpression)(optionalExpression, property.expressions[0]);
        }
        const e = (0, commonSchema_1.callCreateCallExpression)(s, property.identifier, undefined, property.expressions ? property.expressions : [expressionWithProperties]);
        const ex = expressionWithProperties;
        if (property.expressions) {
            // Reduce unnecessary "pipe"
            return ex.expression.escapedText === 'pipe'
                ? Object.assign(Object.assign({}, expressionWithProperties), { arguments: [...ex.arguments, e] }) : (0, commonSchema_1.callPipe)(undefined, [expressionWithProperties, e]);
        }
        return e;
    }, expression);
}
 ...
 */
function buildEffectSchemaObject({ typeNode, S, dependencies, sourceFile, getDependencyName, schemaExtensionClauses, skipParseJSDoc, }) {
    ...
    if (schemaExtensionClauses && schemaExtensionClauses.length > 0) {
        objectSchema = buildEffectSchemaExtendedSchema(S, schemaExtensionClauses, properties.length > 0
            ? [
...
function buildSchemaReference({ node, dependencies, sourceFile, getDependencyName, }, path = '') {
    ...
    if (indexTypeName === '-1') {
        ...
        ts.isIndexedAccessTypeNode(node.objectType)) {
        return buildSchemaReference({ node: node.objectType, dependencies, sourceFile, getDependencyName }, `[${indexTypeName}].${path}`);
    }
    ...
    if (ts.isTypeReferenceNode(node.objectType)) {
        ...
        dependencies.push(dependencyName);
        const e = f.createPropertyAccessExpression((0, commonSchema_1.callCreateCallExpression)('S', 'getPropertySignatures', undefined, [
            f.createIdentifier(dependencyName),
        ]), f.createIdentifier(indexTypeName));
        return path
            ? f.createPropertyAccessExpression((0, commonSchema_1.callCreateCallExpression)('S', 'getPropertySignatures', undefined, [
                e,
