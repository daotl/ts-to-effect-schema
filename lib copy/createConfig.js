...
async function createConfig(configPath) {
    if ((0, fs_extra_1.existsSync)(configPath)) {
        const { answer } = await inquirer_1.default.prompt({
            type: 'confirm',
            name: 'answer',
            message: 'ts-to-effect-schema.config.js already exists, do you want to override it?',
        });
        if (!answer) {
            return false;
        }
    ...
    const getOutputDefault = () => {
        if ((answers === null || answers === void 0 ? void 0 : answers.mode) === 'single') {
            ...
            return answers.config[answers.config.length - 1].input.replace(/\.ts(x)?$/, '.schema.ts$1');
        }
    };
