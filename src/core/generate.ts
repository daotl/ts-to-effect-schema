...
${Array.from(effectSchemasWithMissingDependencies).join('\n')}`,
  ...
  const getEffectSchemasFile = (
    ...
    const genTypeFestModule = () => {
      ...
      if (usedSchemaNames.includes('omitCommonProperties')) {
        modules += modules.length ? ', IsEmptyObject' : 'IsEmptyObject'
      }

      return modules
    }

    const typeFestModule = genTypeFestModule()

    return `// Generated by ts-to-effect-schema
import * as S from "@effect/schema/Schema";
${typeFestModule ? `import type { ${typeFestModule} } from "type-fest";` : ''}${
