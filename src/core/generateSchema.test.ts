...
describe("generateSchema", () => {
  ...
  it("should generate two joined schemas", () => {
    ...
    expect(generate(source)).toMatchInlineSnapshot(`
      "export const supermanWithWeaknessSchema = pipe(supermanSchema, S.extend(omitCommonProperties(S.struct({
          weakness: kryptoniteSchema
      }), supermanSchema)));"
    `);
  });
  ...
  it("should generate an extended schema", () => {
    ...
    expect(generate(source)).toMatchInlineSnapshot(`
      "export const supermanSchema = pipe(clarkSchema, S.extend(omitCommonProperties(S.struct({
          withPower: S.boolean
      }), clarkSchema)));"
    `);
  });
  ...
  it("should generate a merged schema when two extends are used", () => {
    ...
    expect(generate(source)).toMatchInlineSnapshot(`
      "export const supermanSchema = pipe(pipe(clarkSchema, S.extend(omitCommonProperties(kalLSchema, clarkSchema))), S.extend(omitCommonProperties(S.struct({
          withPower: S.boolean
      }), pipe(clarkSchema, S.extend(omitCommonProperties(kalLSchema, clarkSchema))))));"
    `);
  });
  ...
  it("should generate a merged schema when extending with multiple comma-separated interfaces", () => {
    ...
    expect(generate(source)).toMatchInlineSnapshot(`
      "export const supermanSchema = pipe(pipe(pipe(clarkSchema, S.extend(omitCommonProperties(kalLSchema, clarkSchema))), S.extend(omitCommonProperties(kryptonianSchema, pipe(clarkSchema, S.extend(omitCommonProperties(kalLSchema, clarkSchema)))))), S.extend(omitCommonProperties(S.struct({
          withPower: S.boolean
      }), pipe(pipe(clarkSchema, S.extend(omitCommonProperties(kalLSchema, clarkSchema))), S.extend(omitCommonProperties(kryptonianSchema, pipe(clarkSchema, S.extend(omitCommonProperties(kalLSchema, clarkSchema)))))))));"
    `);
  });
  ...
  it("should deal with composed index signature", () => {
    ...
    expect(generate(source)).toMatchInlineSnapshot(`
      "export const moviesSchema = S.record(S.string, movieSchema).and(S.struct({
          "Man of Steel": pipe(movieSchema, S.extend(omitCommonProperties(S.struct({
